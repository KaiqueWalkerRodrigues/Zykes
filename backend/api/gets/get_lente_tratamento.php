...
try {
    ...
    if (!isset($input['id_tratamento_lente'])) {
        http_response_code(400); // 400 Bad Request
        echo json_encode([
            "status"  => "error",
            "message" => "Campo obrigatório: id_tratamento_lente."
        ]);
        exit;
    }

    // Valida se o ID é um número inteiro válido e positivo.
    $id_tratamento_lente = (int)$input['id_tratamento_lente'];
    if ($id_tratamento_lente <= 0) {
        http_response_code(400); // 400 Bad Request
        echo json_encode([
            "status"  => "error",
            "message" => "ID inválido."
    ...
    // Prepara e executa a consulta para buscar um único tratamento que não foi deletado.
    $stmt = $pdo->prepare("
        SELECT id_tratamento_lente, nome, created_at, updated_at, deleted_at
        FROM lente_tratamentos
        WHERE id_tratamento_lente = ? AND deleted_at IS NULL
        LIMIT 1
    ");
    $stmt->execute([$id_tratamento_lente]);
    ...
    $etagBase = ($tratamento['updated_at'] ?? $tratamento['created_at']) . ':' . $tratamento['id_tratamento_lente'];
    ...
    echo json_encode([
        "status"  => "error",
        "message" => "Erro ao buscar o tratamento de lente.",

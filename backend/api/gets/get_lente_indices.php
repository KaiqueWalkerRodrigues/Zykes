...
try {
    ...
    // Busca apenas os índices não deletados
    $stmt = $pdo->query("
        SELECT id_indice_lente, nome, created_at, updated_at
        FROM lente_indices
    ...
    // Uma forma eficiente é usar um hash do resultado.
    $payloadParaHash = json_encode($indices, JSON_UNESCAPED_UNICODE);
    ...
    // Se corresponder, significa que o cliente já tem a versão mais recente dos dados.
    $ifNoneMatch = $_SERVER['HTTP_IF_NONE_MATCH'] ?? null;
    if ($ifNoneMatch && trim($ifNoneMatch) === $etag) {
        // Retorna 304 Not Modified, informando que não há alterações. O corpo da resposta fica vazio.
    ...
    echo json_encode([
        "status"  => "error",
        "message" => "Erro ao buscar os índices de refração.",
